...
extern int TypeStop=1;
extern bool TradeBuyWithStop=true;
extern double ValueBuySL=200;
extern bool TradeSellWithStop=true;
extern double ValueSellSL=200;
...
if ((TradeBuyWithStop==true)&&(ReCountBuy!=0)&&(ReCountBuy>CountBuy)&&(TypeStop==1)){CheckBuyStopForFirstOrder();}
if ((TradeSellWithStop==true)&&(ReCountSell!=0)&&(ReCountSell>CountSell)&&(TypeStop==1)){CheckSellStopForFirstOrder();}
if ((TradeBuyWithStop==true)&&(ReCountBuy!=0)&&(ReCountBuy>CountBuy)&&(TypeStop==0)){CheckBuyStopForEveryOrder();}
if ((TradeSellWithStop==true)&&(ReCountSell!=0)&&(ReCountSell>CountSell)&&(TypeStop==0)){CheckSellStopForEveryOrder();}
      ...
      if(OrderSelect(i7,SELECT_BY_POS)==true)
        {
         if((OrderSymbol()==Symbol())&&(OrderType()==OP_BUY)) 
           ...
           if (CheckB!=(TPBuy*Point*k)){
           double TPBUpd=OrderOpenPrice()+TPBuy*Point*k;
      ...
      if(OrderSelect(i8,SELECT_BY_POS)==true)
        {
         if((OrderSymbol()==Symbol())&&(OrderType()==OP_SELL)) 
           ...
           if (CheckS!=(TPSell*Point*k)){
           
           double TPSUpd=OrderOpenPrice()-TPSell*Point*k;
   ...
   if((CountBuy==0) && (BuyTrade==true))
     {
      ...
      if(IsTradeAllowed()) 
        {
         if(OrderSend(Symbol(),OP_BUY,LotBuy1,Ask,3*k,NULL,NULL,"Sun-LotBuy1-buy(1)",Magic_Number,0,Blue)<0)
   ...
   if((CountSell==0) && (SellTrade==true))
     {
      ...
      if(IsTradeAllowed()) 
        {
         if(OrderSend(Symbol(),OP_SELL,LotSell1,Bid,3*k,NULL,NULL,"Sun-LotSell1-sell(1)",Magic_Number,0,Red)<0)
           {Alert("Ошибка открытия позиции № ",GetLastError()); }
        }
     }
...
double CalculateBuyTP()
  {
   ...
   TPB=PriceB/CountB+TPBuy*Point*k;
   return(TPB);
  }
//#Рассчёт итогового профита ордеров на покупку
...
TPB=0;
   ...
   if(CountB>1)
     {
      TPB=PriceB/BuyLots+TPBuy*Point*k;
 ...
 double CalculateTotalSellTP2()
  {
   ...
   if(CountS>1)
     {
      TPS=PriceS/SellLots-TPSell*Point*k;
...
double CalculateSellTP()
  {
   ...
   if(CountS!=0)
     {
      TPS=PriceS/CountS-TPSell*Point*k; 
     }
   return(TPS);
  }
...
double SellLimDel()
  ...
  double CheckBuyStopForFirstOrder() {
SearchFirstBuyOrder();
OrderSelect(Ticket, SELECT_BY_TICKET); 
double StartBuyPrice=OrderOpenPrice();
 double StopB=StartBuyPrice-ValueBuySL*Point*k;
      ...
      if(OrderSelect(iFBSearch,SELECT_BY_POS)==true)
        {
         if(( OrderSymbol()==Symbol()) && (OrderType()==OP_BUY))
           {
            if(OrderStopLoss()==NULL){ OrderModify(OrderTicket(),OrderOpenPrice(),StopB,NULL,0,Orange);}
           }
        }
     }
   return(0);
    ...
    double CheckSellStopForFirstOrder() {
 SearchFirstSellOrder();
OrderSelect(Ticket, SELECT_BY_TICKET); 
double StartSellPrice=OrderOpenPrice();
 double StopS=StartSellPrice+ValueSellSL*Point*k;
      ...
      if(OrderSelect(iFSSearch,SELECT_BY_POS)==true)
        {
         if(( OrderSymbol()==Symbol()) && (OrderType()==OP_SELL))
           {
            if(OrderStopLoss()==NULL){ OrderModify(OrderTicket(),OrderOpenPrice(),StopS,NULL,0,Orange);}
           }
        }
     }
   return(0);
  ...
  double CheckBuyStopForEveryOrder() {
SearchFirstBuyOrder();

OrderSelect(Ticket, SELECT_BY_TICKET); 
double StartBuyPrice2=OrderOpenPrice();
double StopB2=0;
   for(int iFBSearch=0;iFBSearch<OrdersTotal();iFBSearch++)
     {
      if(OrderSelect(iFBSearch,SELECT_BY_POS)==true)
        {
         if(( OrderSymbol()==Symbol()) && (OrderType()==OP_BUY))
           {
            if(OrderStopLoss()==NULL){ StopB2=OrderOpenPrice()-ValueBuySL*k*Point;OrderModify(OrderTicket(),OrderOpenPrice(),StopB2,NULL,0,Orange);}
           }
        }
     }
   return(0);
    ...
    double CheckSellStopForEveryOrder() {
 SearchFirstSellOrder();

OrderSelect(Ticket, SELECT_BY_TICKET); 
double StartSellPrice2=OrderOpenPrice();
double StopS2=0;
   for(int iFSSSearch=0;iFSSSearch<OrdersTotal();iFSSSearch++)
     {
      if(OrderSelect(iFSSSearch,SELECT_BY_POS)==true)
        {
         if(( OrderSymbol()==Symbol()) && (OrderType()==OP_SELL))
           {
            if(OrderStopLoss()==NULL){ StopS2=OrderOpenPrice()+ValueSellSL*k*Point;OrderModify(OrderTicket(),OrderOpenPrice(),StopS2,NULL,0,Orange);}
           }
        }
     }
   return(0);
